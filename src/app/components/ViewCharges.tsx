...
interface ViewChargesProps {
  requestId: string;
  reference: string;
  onClose: () => void;
}

export function ViewCharges({ requestId, reference, onClose }: ViewChargesProps) {
  const [accountEntries, setAccountEntries] = useState<AccountEntry[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Mock charge data - in production, this would come from an API
  const chargeDetails = {
    channelReference: `${requestId}-PGW24`,
    walletReference: `1E0557674596501QDU6kWALREF${reference.slice(-14)}`,
    idNo: '27905634',
  };
  ...
  // Fetch account entries from API
  useEffect(() => {
